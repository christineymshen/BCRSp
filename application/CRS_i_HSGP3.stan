...
transformed parameters {
  ...
  vector[M_nD0[2]] sqrt_SPD02 = sqrt_diagSPD_2D(alpha0[2], l0[2], L0[,2], idx_basis02);
  {
    ...
    matrix[n,m] theta0;
    theta0[,1] = PHI01 * (sqrt_SPD01 .* z01);
    theta0[,2] = PHI02 * (sqrt_SPD02 .* z02);
    matrix[n,m] Xbeta = X * beta + theta0;
...
generated quantities {
  array[m] vector[n_pred] theta_pred;
  vector<lower=0>[m] alpha0_adj;
  {
    ...
    if (M_nD0[1]>n){
      ...
      theta_pred[1] = theta_pred_rng(PHI01_sqrt_SPD01, PHI01_pred_sqrt_SPD01, z01);
    } else {
      theta_pred[1] = PHI01_pred * (sqrt_SPD01 .* z01);
    }
    if (M_nD0[2]>n){
      matrix[n_pred,M_nD0[2]] PHI02_pred_sqrt_SPD02 = diag_post_multiply(PHI02_pred, sqrt_SPD02);
      theta_pred[2] = theta_pred_rng(PHI02_sqrt_SPD02, PHI02_pred_sqrt_SPD02, z02); 
    } else {
      theta_pred[2] = PHI02_pred * (sqrt_SPD02 .* z02);
    }
  }
}
