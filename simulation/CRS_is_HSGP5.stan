functions{
  ...
  vector theta_pred_rng(matrix PHI_sqrt_SPD, matrix PHI_pred_sqrt_SPD, vector z_obs){
    ...
    vector[m] b = to_vector(normal_rng(rep_vector(0, m), rep_vector(1, m)));
		vector[n_pred] theta_pred = PHI_pred_sqrt_SPD * (QQT * z_obs + add_diag(-QQT,1) * b);
    return theta_pred;
  }
}
data {
...
parameters {
  ...
  matrix<lower=0>[m,k] psi;
	...
	vector<lower=0>[m] l0; // GP length scale for intercept
 	vector<lower=0>[m] alpha1; // GP scale for intercept
	vector<lower=0>[m] l1; // GP length scale for intercept
  vector[M_nD0[1]] z01;
  vector[M_nD0[2]] z02;
  vector[M_nD1[1]] z11;
  vector[M_nD1[2]] z12;
...
model {
  ...
  target += normal_lupdf(alpha0 |0,4);
	target += normal_lupdf(alpha1 |0,4);
